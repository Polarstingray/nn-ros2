...
class NNController(Node):
    def __init__(self):
        ...
        default_model_path = os.path.join(
            os.path.dirname(__file__), '..', 'resource', 'resnet50_model.pth') # this might need to be adjusted
        self.declare_parameter('model_path', os.path.abspath(default_model_path))
    ...
    def image_callback(self, msg):
        try:
            ...
            # Extract and clamp velocities to Waffle Pi hardware limits
            linear_x  = max(-0.26, min(0.26,  float(output[0][0].item())))
            angular_z = max(-1.82, min(1.82,  float(output[0][1].item())))
